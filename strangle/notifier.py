...
class NotificationManager:
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # NEW: Risk Management Notifications
    # ═══════════════════════════════════════════════════════════════════════

    def notify_daily_stop(self, daily_pnl: float, threshold: float):
        """Notification when daily loss kill-switch is triggered"""
        message = (
            f"🛑 <b>DAILY LOSS KILL-SWITCH TRIGGERED</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"<b>Daily P&L:</b> ₹{daily_pnl:,.2f}\n"
            f"<b>Threshold:</b> ₹{threshold:,.2f}\n"
            f"\n"
            f"⚠️ <b>ALL POSITIONS WILL BE CLOSED</b>\n"
            f"⚠️ <b>NO NEW ENTRIES TODAY</b>\n"
            f"\n"
            f"⏰ Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S IST')}"
        )
        ...
        print(f"{Fore.RED}🛑 DAILY LOSS KILL-SWITCH TRIGGERED{Style.RESET_ALL}")
        print(f"{Fore.RED}Daily P&L: ₹{daily_pnl:,.2f} (Threshold: ₹{threshold:,.2f}){Style.RESET_ALL}")
        print(f"{Fore.RED}ALL POSITIONS WILL BE CLOSED{Style.RESET_ALL}")
    ...
    def notify_vix_shock(self, prev_vix: float, current_vix: float, action_summary: str):
        ...
        vix_roc_pct = (vix_change / prev_vix * 100) if prev_vix > 0 else 0

        message = (
            f"⚠️ <b>VIX SHOCK DETECTED</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"<b>Previous VIX:</b> {prev_vix:.2f}\n"
            f"<b>Current VIX:</b> {current_vix:.2f}\n"
            f"<b>Change:</b> +{vix_change:.2f} pts ({vix_roc_pct:+.1f}%)\n"
            f"\n"
            f"<b>Actions Taken:</b>\n"
            f"{action_summary}\n"
            f"\n"
        ...
        self.send_alert(message, "⚠️ VIX SHOCK")

        # Console alert
        print(f"\n{Fore.YELLOW}{'=' * 80}{Style.RESET_ALL}")
        print(f"{Fore.YELLOW}⚠️ VIX SHOCK DETECTED{Style.RESET_ALL}")
        print(f"{Fore.YELLOW}VIX: {prev_vix:.2f} → {current_vix:.2f} (+{vix_change:.2f} pts){Style.RESET_ALL}")
        print(f"{Fore.YELLOW}Actions: {action_summary}{Style.RESET_ALL}")
    ...
    def notify_delta_hedge(self, net_delta_before: float, net_delta_after: float, 
        ...
        message = (
            f"🔄 <b>DELTA HEDGE EXECUTED</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"<b>Net Delta Before:</b> {net_delta_before:.0f}\n"
            f"<b>Net Delta After:</b> {net_delta_after:.0f}\n"
            f"<b>Change:</b> {delta_change:+.0f}\n"
            f"\n"
        ...
        self.send_alert(message, "🔄 HEDGE")

        # Console alert
        print(f"\n{Fore.CYAN}{'=' * 80}{Style.RESET_ALL}")
        print(f"{Fore.CYAN}🔄 DELTA HEDGE EXECUTED{Style.RESET_ALL}")
        print(f"{Fore.CYAN}Net Delta: {net_delta_before:.0f} → {net_delta_after:.0f}{Style.RESET_ALL}")
        print(f"{Fore.CYAN}Hedge: {instruments}{Style.RESET_ALL}")
