"""
short_strangle_system_V4_PRODUCTION.py
...
class Config:
    ...
    MIN_COMBINED_PREMIUM = 150
    MAX_COMBINED_PREMIUM = 300
...
class NotificationManager:
    ...
    def send_daily_summary(self, metrics: Any):
        message = (
            ...
            f"CE P&L: ₹{metrics.ce_pnl:,.2f}\n"
            f"PE P&L: ₹{metrics.pe_pnl:,.2f}\n"
...
class BrokerInterface:
    ...
    def place_order(self, symbol: str, qty: int, direction: Direction, price: float) -> str:
        if self.backtest_data is not None:
            logging.info(f"Simulated order: {direction.value} {qty} of {symbol} @ ₹{price:.2f}")
...
class TradeManager:
    ...
    def add_trade(self, trade: Trade):
        ...
        self.notifier.send_alert(
            f"New Trade: {trade.direction.value} {trade.qty} {trade.symbol} @ ₹{trade.entry_price:.2f}",
            "INFO"
        )
    ...
    def close_trade(self, trade_id: str, exit_price: float):
        if trade_id in self.active_trades:
            ...
            self.notifier.send_alert(
                f"Closed Trade: {trade.symbol}, P&L: ₹{pnl:,.2f}",
...
class ShortStrangleStrategy:
    ...
    def should_enter_trade(self) -> Tuple[bool, str]:
        ...
        if vix < Config.VIX_LOW_THRESHOLD:
            return False, f"VIX too low ({vix:.2f} < {Config.VIX_LOW_THRESHOLD})"
        if vix > Config.VIX_HIGH_THRESHOLD * 1.5:
            return False, f"VIX extremely high ({vix:.2f} > {Config.VIX_HIGH_THRESHOLD * 1.5})"
        if iv_percentile < Config.MIN_IV_PERCENTILE:
            return False, f"IV percentile too low ({iv_percentile:.1f}% < {Config.MIN_IV_PERCENTILE}%)"
        if iv_percentile > Config.MAX_IV_PERCENTILE:
            return False, f"IV percentile too high ({iv_percentile:.1f}% > {Config.MAX_IV_PERCENTILE}%)"
        if len(self.trade_manager.active_trades) >= 2:
            return False, "Already have active positions"

        return True, f"Entry approved - VIX: {vix:.2f}, IV Percentile: {iv_percentile:.1f}%"
    ...
    def select_strike(self, current_date: Optional[date] = None) -> Tuple[str, str]:
        ...
        logging.info(f"Selected strikes - CE: {ce_strike}, PE: {pe_strike}, OTM Distance: {otm_distance}, VIX: {vix:.2f}")
    ...
    def check_trailing_stop(self, trade: Trade) -> bool:
        ...
        if trade.trailing_stop_price is not None:
            ...
            if current_pnl < trade.trailing_stop_price:
                logging.info(f"Trailing stop hit for {trade.symbol}")
                return True
        return False
    ...
    def check_profit_target(self, trade: Trade) -> bool:
        ...
        if pnl_pct >= Config.PROFIT_TARGET_PCT * 100:
            logging.info(f"Profit target reached for {trade.symbol}: {pnl_pct:.1f}%")
            return True
        return False
    ...
    def should_roll_position(self, trade: Trade) -> bool:
        ...
        if pnl_pct <= -Config.ROLL_THRESHOLD_PCT * 100 and trade.rolled_from is None:
            logging.info(f"Roll threshold reached for {trade.symbol}: {pnl_pct:.1f}%")
            return True
        return False
    ...
    def roll_position(self, trade: Trade, current_date: Optional[date] = None):
        ...
        if new_price > 0:
            ...
            if order_id:
                ...
                logging.info(f"Rolled position from {trade.symbol} to {new_symbol}")
    ...
    def manage_active_positions(self, backtest_timestamp: Optional[datetime] = None):
        ...
        for option_type in ["CE", "PE"]:
            if self.trade_manager.check_leg_stop_loss(option_type):
                logging.warning(f"{option_type} leg hit max loss threshold - exiting all positions")
                for trade_id in list(self.trade_manager.active_trades.keys()):
                    trade = self.trade_manager.active_trades[trade_id]
                    exit_price = self.broker.get_quote(trade.symbol)
                    if exit_price > 0:
...
class ConsoleDashboard:
    ...
    def render(self, market_data: MarketData, trade_manager: TradeManager):
        ...
        data = [
            ...
            ["Daily P&L", f"₹{trade_manager.daily_pnl:,.2f}"],
            ["CE Leg P&L", f"₹{trade_manager.ce_pnl:,.2f}"],
            ["PE Leg P&L", f"₹{trade_manager.pe_pnl:,.2f}"],
        ...
        if trade_manager.active_trades:
            ...
            for trade in trade_manager.active_trades.values():
                pos_data.append([
                    ...
                    f"₹{trade.get_pnl():,.2f}",
                    f"₹{trade.trailing_stop_price:.2f}" if trade.trailing_stop_price else "N/A"
                ])
...
""")
    ...
    def calculate_strikes(spot: float, vix: float, current_date: date) -> Tuple[float, float, date]:
        ...
        expiry = (pd.to_datetime(current_date) + pd.Timedelta(
            days=7 - pd.to_datetime(current_date).weekday())).date()
    ...
    for current_date in pd.date_range(start_date, end_date, freq='D'):
        ...
        print(f"\n{Fore.YELLOW}Trading Day: {current_date.strftime('%Y-%m-%d')}{Style.RESET_ALL}")

        # Reset daily entry flag
        strategy.entry_allowed_today = True

        # Simulate intraday trading
        for index in range(len(daily_data)):
            broker.current_index = daily_data.index[index]
            current_time = daily_data.iloc[index]['timestamp']
        ...
        logging.info(
            ...
            f"Daily P&L: ₹{trade_manager.daily_pnl:,.2f}, "
            f"CE: ₹{metrics.ce_pnl:,.2f}, "
            f"PE: ₹{metrics.pe_pnl:,.2f}"
        )
    ...
    summary_data = [
        ...
        ["Cumulative P&L", f"₹{cumulative_pnl:,.2f}"],
        ["Max Drawdown", f"₹{final_metrics.max_drawdown:,.2f}"],
        ...
        ["Avg Daily P&L", f"₹{cumulative_pnl / len(trade_manager.daily_pnl_history):,.2f}" if trade_manager.daily_pnl_history else "N/A"]
    ]
    ...
    if trade_manager.daily_pnl_history:
        ...
        print(f"  Min Daily P&L: ₹{np.min(daily_pnl_array):,.2f}")
        print(f"  Max Daily P&L: ₹{np.max(daily_pnl_array):,.2f}")
        print(f"  Median Daily P&L: ₹{np.median(daily_pnl_array):,.2f}")
        print(f"  Std Dev: ₹{np.std(daily_pnl_array):,.2f}")
...
{Fore.CYAN}  Version 4.0 - Production Backtest Edition  {Style.RESET_ALL}
{Fore.CYAN}{'=' * 80}{Style.RESET_ALL}
""")
