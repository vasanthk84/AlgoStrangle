...
FIXED: Added deduplication to best/worst trades list
FIXED: Replaced Best/Worst Trades with a Full Trade Log
"""
...
class BacktestAnalyzer:
    ...
    # ---
    # FIX: Replaced best_worst_trades with print_all_trades_summary
    # ---
    def print_all_trades_summary(self):
        """Show a full list of all trades taken and their MTM/P&L"""
        if self.trades_df.empty:
            return

        print(f"\n{Fore.CYAN}{'=' * 80}{Style.RESET_ALL}")
        print(f"{Fore.CYAN}FULL TRADE LOG (Sorted by Entry Time){Style.RESET_ALL}")
        print(f"{Fore.CYAN}{'=' * 80}{Style.RESET_ALL}\n")

        completed = self.trades_df[self.trades_df['pnl'].notna()].copy()

        # Deduplicate the trades DataFrame based on key columns
        key_columns = ['symbol', 'entry_time', 'exit_time', 'entry_price', 'exit_price', 'pnl']
        completed.drop_duplicates(subset=key_columns, inplace=True, keep='first')

        # Sort by entry time to see the log in chronological order
        all_trades_sorted = completed.sort_values('entry_time')

        trade_data = []
        for _, trade in all_trades_sorted.iterrows():
            # Color P&L
            pnl_val = trade['pnl']
            color = Fore.GREEN if pnl_val >= 0 else Fore.RED
            pnl_str = f"{color}₹{pnl_val:,.2f}{Style.RESET_ALL}"
        ...
        print(tabulate(trade_data,
                      headers=["Date", "Symbol", "Type", "Entry", "Exit", "Exit Reason", "P&L"],
                      tablefmt="simple"))
    # --- End Fix ---
